...
#######
{
  ...
  # Regression test: machinesFiltered must still allow reading public var values.
  # The builtins.path filter that strips vars/ from settings.directory must not
  # prevent direct file reads of committed var values via varsDirectory.
  test_machines_filtered_reads_public_vars =
    let
      eval = clan {
        self = {
          inputs = { };
        };
        directory = ./cycle-test-flake;
        meta.name = "test";
        ...
        machines.test-machine = {
          ...
          clan.core.vars.generators.my-gen = {
            # Same as in the next test
            ...
            script = "echo test > $out/my-public-file";
          };
        };
      };

      filteredConfig = eval.config.clanInternals.machinesFiltered.x86_64-linux.test-machine.config;
      unfilteredConfig = eval.config.clanInternals.machines.x86_64-linux.test-machine.config;
    in
    {
      expr = {
        # Both filtered and unfiltered should see the public var file
        filteredValue = filteredConfig.clan.core.vars.generators.my-gen.files.my-public-file.value;
        unfilteredValue = unfilteredConfig.clan.core.vars.generators.my-gen.files.my-public-file.value;
        filteredExists = filteredConfig.clan.core.vars.generators.my-gen.files.my-public-file.exists;
        unfilteredExists = unfilteredConfig.clan.core.vars.generators.my-gen.files.my-public-file.exists;

      };
      expected = {
        filteredValue = "hello-from-var";
        unfilteredValue = "hello-from-var";
        filteredExists = true;
        unfilteredExists = true;
      };
    };

  machines_filtered_readfile_public_vars =
    let
      eval = clan {
        self = {
          inputs = { };
    ...
    in
    {
      test_config_access = {
        expr = builtins.readFile (
          unfilteredConfig.clan.core.settings.directory
          + "/vars/per-machine/test-machine/my-gen/my-public-file/value"
        );
        expected = "hello-from-var";
      };
      test_readfile_access = {
        # Cannot open the file
        expr = builtins.readFile (
          filteredConfig.clan.core.settings.directory
          + "/vars/per-machine/test-machine/my-gen/my-public-file/value"
        );
        expectedError = {
          msg = "opening file";
      ...
      # Test that secrets exist
      test_exist_secret = {
        expr = {
          normal = builtins.pathExists (
            unfilteredConfig.clan.core.settings.directory
            + "/vars/per-machine/test-machine/my-gen/my-secret-file"
          );
          filtered = builtins.pathExists (
            filteredConfig.clan.core.settings.directory + "/vars/per-machine/test-machine/my-gen/my-secret-file"
          );
        };
        expected = {
          normal = true;
