    ...
    { ... }:
    {
      clan.nixosTests.nixos-test-age-backend = {
            ...
            in
            {
              ...
              system.activationScripts.mockAgeUpload = {
                text = ''
                  ...
                  mkdir -p /etc/secret-vars/activation/per-machine/machine/test-generator
                  ${pkgs.age}/bin/age --decrypt -i /etc/secret-vars/key.txt \
                    -o /etc/secret-vars/activation/per-machine/machine/test-generator/activation-secret \
                    ${./secrets/clan-vars/per-machine/machine/test-generator/activation-secret/activation-secret.age}
                  chmod 400 /etc/secret-vars/activation/per-machine/machine/test-generator/activation-secret
                '';
                deps = [ "specialfs" ];
              };
        ...
        testScript = ''
          ...
          result = machine.succeed("cat /run/secrets/per-machine/machine/test-generator/service-secret").strip()
          ...
          result = machine.succeed("cat /run/secrets/shared/shared-generator/shared-secret").strip()
          ...
          result = machine.succeed("cat /run/user-secrets/per-machine/machine/test-generator/user-secret").strip()
          ...
          result = machine.succeed("cat /etc/secret-vars/activation/per-machine/machine/test-generator/activation-secret").strip()
          ...
          stat_result = machine.succeed("stat -c '%U:%G' /run/secrets/per-machine/machine/perm-generator/perm-secret").strip()
          ...
          mode_result = machine.succeed("stat -c '%a' /run/secrets/per-machine/machine/perm-generator/perm-secret").strip()
          assert mode_result == "440", f"perm-secret mode: expected '440', got '{mode_result}'"
          print("✓ Custom mode 0440 applied")

          default_mode = machine.succeed("stat -c '%a' /run/secrets/per-machine/machine/test-generator/service-secret").strip()
