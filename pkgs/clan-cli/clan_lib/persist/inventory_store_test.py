...
def test_simple_read_write(setup_test_files: Path) -> None:
    ...
    data: dict = store.read()  # type: ignore[misc]
    assert data == {"foo": "bar", "protected": "protected"}

    set_value_by_path(data, "foo", "foo")  # type: ignore[misc]
    store.write(data, "test", commit=False)  # type: ignore[misc]
    ...
    with pytest.raises(ClanError) as e:
        store.write(invalid_data, "test", commit=False)  # type: ignore[misc]
    assert (
        str(e.value)
...
def test_simple_deferred(setup_test_files: Path) -> None:
    ...
    delete_by_path(data, "foo.c")  # type: ignore[misc]
