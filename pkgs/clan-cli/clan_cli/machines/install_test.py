...
from clan_cli.machines.install import INSTALL_WARNING
...
@pytest.mark.broken_on_darwin
@pytest.mark.with_core
def test_install_prints_destructive_warning_by_default(
    monkeypatch: pytest.MonkeyPatch,
    flake: ClanFlake,
    ...
    with patch("clan_cli.machines.install.run_machine_install"):
        cli.run(
            [
                "machines",
                "install",
    ...
    assert INSTALL_WARNING in output
