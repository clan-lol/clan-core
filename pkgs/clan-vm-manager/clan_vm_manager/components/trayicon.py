...
class Win32Implementation(BaseImplementation):
    ...
    def __init__(self, application: Gtk.Application) -> None:
        from ctypes import windll  # type: ignore[misc]
    ...
    def _register_class(self) -> None:
        from ctypes import byref, windll  # type: ignore[misc]

        self._window_class = self.WNDCLASSW(  # type: ignore[misc]
            style=(self.CS_VREDRAW | self.CS_HREDRAW),
            lpfn_wnd_proc=self.WNDCLASSW.LPFN_WND_PROC(self.on_process_window_message),  # type: ignore[misc]
