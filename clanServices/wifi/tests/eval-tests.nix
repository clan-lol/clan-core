...
let
  ...
  # evaluatedService =
  #   testFlake.clanInternals.inventoryClass.distributedServices.servicesEval.config.allServices.self-zerotier-redux.config;
in
{
  test_simple = {
    inherit testFlake;
    ...
    expr =
      testFlake.config.clan.clanInternals.inventoryClass.distributedServices.servicesEval.config.allServices.self-wifi.config;
    expected = 1;
