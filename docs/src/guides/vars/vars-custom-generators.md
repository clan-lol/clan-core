...
Each generator directory also carries its `.validation-hash`, so that vars
restored by `clan vars import` are not considered outdated and regenerated by
the next `clan vars generate`.

!!! warning
    The exported files contain **unencrypted secrets**. The dump is created with owner-only permissions (`0700` directories, `0600` files), but it is still plaintext: handle the export folder with care and delete it after use.
